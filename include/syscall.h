...
typedef uint64_t (*syscall_t)(void);

// Definition for syscall jump table
extern syscall_t syscall_table[NUM_SYSCALLS];

static inline uint64_t execute_syscall(uint64_t num, uint64_t arg1, uint64_t arg2, uint64_t arg3, uint64_t arg4)
{
	...
	uint64_t rv, dummy_rdi, dummy_rsi, dummy_rdx, dummy_rcx;
	asm volatile (
		"syscall\n"
