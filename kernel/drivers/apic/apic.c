...
static void add_ioapic(struct madt_entry_ioapic *entry)
{
	...
	vmm_map_page(kernel_p4, entry->phys_addr, phys_to_virt(entry->phys_addr), VMM_ALLOC_MMAP | PAGE_GLOBAL | PAGE_DISABLE_CACHE | PAGE_WRITABLE);
...
static void parse_madt(struct acpi_madt *madt)
{
	...
	vmm_map_page(kernel_p4, virt_to_phys(lapic_base), lapic_base, VMM_ALLOC_MMAP | PAGE_GLOBAL | PAGE_DISABLE_CACHE | PAGE_WRITABLE);
}
