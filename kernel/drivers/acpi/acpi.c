...
static void map_table(struct acpi_header *hd)
{
	...
	vmm_map_page(kernel_p4, virt_to_phys(hd), hd, VMM_ALLOC_MMAP);
	vmm_map_page(kernel_p4, virt_to_phys(hd) + sizeof(struct acpi_header),
			(virtaddr_t)((uintptr_t)hd + sizeof(struct acpi_header)), VMM_ALLOC_MMAP);
	...
	for (size_t i = sizeof(struct acpi_header) + PAGE_SIZE; i < hd->length; i += PAGE_SIZE) {
		...
		vmm_map_page(kernel_p4, phys, virt, VMM_ALLOC_MMAP);
	}
